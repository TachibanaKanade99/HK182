...
int translate(address_t virtual, address_t *physical) {
	...
	//printf("Index of virtual add: %u\n", get_index(virtual));

	for (int i = 0; i < TLB_SIZE; i++){
		if (get_index(virtual) == tlb[i].virtual){
			tlb[i].count++;
	...
	for (int j = 0; j < PAGE_TABLE_SIZE; j++){
		if (get_index(virtual) == page_table[j].virtual){
			page_table[j].count++;
			*physical = (page_table[j].physical << OFF_LEN) + get_offset(virtual);
			return 1;
		}
	}
	return 0;
